...
class StepCaseLoader:
    def load(self, case_path: str | Path) -> dict[str, Any]:
        case_file = Path(case_path)
        raw_payload = yaml.load(case_file.read_text(encoding="utf-8-sig"), Loader=_NoDuplicateSafeLoader)
        if not isinstance(raw_payload, dict):
            raise ValueError("步骤式 case 顶层必须是字典结构。")
    ...
    def _normalize_case(self, raw_payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(raw_steps, list) or not raw_steps:
            raise ValueError("步骤式 case 必须至少提供一个步骤。")
        return {
    ...
    def _normalize_data(self, raw_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(raw_data, dict):
            raise ValueError("步骤式 case 的数据必须是字典结构。")
        normalized = {}
    ...
    def _normalize_variant_params(self, raw_params: Any) -> list[dict[str, str]]:
        ...
        return [
            {"param": PARAM_KEY_ALIASES.get(label, label), "param_label": label}
            for label in labels
            if label
        ]
    ...
    def _normalize_step(self, raw_step: dict[str, Any]) -> dict[str, Any]:
        ...
        has_direct_target = bool(
            step.get("action") or step.get("element") or step.get("button") or step.get("value") or step.get("text")
        )
        action_params: dict[str, Any] = {}
        field_params: dict[str, Any] = {}
        if has_direct_target:
            action_params = {key: self._normalize_value(value) for key, value in raw_params.items()}
        else:
            field_params = {self._normalize_scalar(key): self._normalize_value(value) for key, value in raw_params.items()}
        return {
    ...
    def _normalize_assertions(self, raw_assertions: Any) -> list[str]:
        ...
        raise ValueError("步骤断言必须是字符串或列表。")
