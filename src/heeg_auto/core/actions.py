...
from heeg_auto.core.duration_utils import derive_duration_range, parse_duration_seconds
...
ACTION_ALIASES = {
    ...
    "assert_duration_in_range": "assert_duration_in_range",
    "断言时长范围": "assert_duration_in_range",
    "assert_control_enabled": "assert_control_enabled",
    "断言控件可用": "assert_control_enabled",
...
class ActionExecutor:
    ...
    def assert_duration_in_range(
        ...
        **kwargs: dict,
        ...
        min_duration = min_duration or kwargs.get("最小时长")
        max_duration = max_duration or kwargs.get("最大时长")
        if not min_duration or not max_duration:
            min_duration, max_duration = self._resolve_duration_range(
                min_duration=min_duration,
                max_duration=max_duration,
                kwargs=kwargs,
            )
        if not min_duration or not max_duration:
            raise ValueError("断言时长范围必须提供最小时长/最大时长，或提供开始/结束时间用于自动推导。")
        if window in (None, ""):
            self.main_page.assert_duration_in_range(
                min_duration=min_duration,
                max_duration=max_duration,
                timeout=timeout,
            )
        ...
        wrapper = self.main_page.find(window_locator, timeout=timeout)
        scoped_page = BasePage(driver=self.driver, logger=self.logger, root=wrapper)
        scoped_page.assert_duration_in_range(
            min_duration=min_duration,
            max_duration=max_duration,
            timeout=timeout,
        )

    def assert_control_enabled(self, target, timeout: int = DEFAULT_TIMEOUT, window=None, **_: dict) -> None:
        locator = self.resolve_target(target)
        page = self._page_for_target(locator, window=window)
        page.assert_control_enabled(locator, timeout=timeout)
    ...
    def assert_latest_clipped_record(
        ...
        **kwargs: dict,
        ...
        if (not min_duration or not max_duration) and not expected_duration:
            min_duration, max_duration = self._resolve_duration_range(
                min_duration=min_duration,
                max_duration=max_duration,
                kwargs=kwargs,
            )
        if not record_name or (not expected_duration and (not min_duration or not max_duration)):
            raise ValueError("断言最新剪辑记录必须提供记录名称，以及预期时长或可推导的开始/结束时间。")
    ...
    def _parse_duration_seconds(raw_value: Any, field_name: str) -> float:
        return parse_duration_seconds(raw_value, field_name=field_name)
    ...
    def _resolve_duration_range(
        ...
        *,
        ...
        start_time = kwargs.get("开始时间") or kwargs.get("start_time") or kwargs.get("剪辑开始时间")
        end_time = kwargs.get("结束时间") or kwargs.get("end_time") or kwargs.get("剪辑结束时间")
        if not start_time or not end_time:
            return min_duration, max_duration
        tolerance_seconds = (
            kwargs.get("时长容差秒数")
            or kwargs.get("容差秒数")
            or kwargs.get("duration_tolerance_seconds")
            or kwargs.get("duration_tolerance")
        )
        return derive_duration_range(
            start_time,
            end_time,
