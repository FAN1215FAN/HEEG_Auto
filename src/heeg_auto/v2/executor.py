...
from heeg_auto.runner.case_loader import CONTEXT_KEY_ALIASES
...
class V2CaseExecutor:
    def __init__(self, asset_store: V2AssetStore | None = None) -> None:
        self.asset_store = asset_store or V2AssetStore()
    ...
    def run_case(
        ...
        for execution in plan:
            ...
            if result["status"] == "FAIL" and case_data.get("stop_on_failure", False):
                halt_reason = "前序执行失败，后续轮次按失败即停标记为未执行。"
    ...
    def _run_execution(
        ...
        for index, step in enumerate(steps, start=1):
            try:
                ...
                failure = {
                    "module_id": "V2步骤式",
                    "failed_step": step.get("step_name") or f"step_{index}",
                }
                step_results.append(
                    {
                        "sequence": index,
        ...
        try:
            if self._is_form_fill_step(step):
                ...
                if progress_callback:
                    progress_callback(f"V2步骤:{step_name}:开始")
                getattr(actions, action_id)(**payload)
                self._merge_step_context(context, step)
            assertion_results = self._run_step_assertions(
                actions,
                step,
                context=context,
                progress_callback=progress_callback,
            ...
            if step.get("optional"):
                return {
                    ...
                    "error_summary": "步骤失败但标记为可选，已跳过。",
                    "assertion_results": [],
                }
            raise
        if progress_callback:
            progress_callback(f"V2步骤:{step_name}:完成")
        return {
            "sequence": sequence,
            "step_name": step_name,
            "status": "PASS",
    ...
    def _run_form_fill_step(self, actions, step: dict[str, Any], progress_callback: ProgressCallback | None = None) -> None:
        ...
        if not window:
            raise ValueError(f"步骤 {step_name} 缺少窗口，无法填写参数。")
        for field_label, field_value in step.get("field_params", {}).items():
            ...
            if progress_callback:
                progress_callback(f"V2步骤:{step_name}:{field_label}")
            getattr(actions, action_id)(**payload)
    ...
    def _build_field_payload(self, window: str, field_label: str, field_value: Any) -> tuple[str, dict[str, Any]]:
        ...
        raise ValueError(f"字段 {field_label} 无法根据控件类型推断填写动作：{target}")
    ...
    def _run_step_assertions(
        ...
        for assertion_name in step.get("assertions", []):
            ...
            for check in checks:
                ...
                started = perf_counter()
                if progress_callback:
                    progress_callback(f"V2断言:{assertion_asset['label']}:开始")
                getattr(actions, action_id)(**payload)
                if progress_callback:
                    progress_callback(f"V2断言:{assertion_asset['label']}:完成")
                results.append(
                    {
