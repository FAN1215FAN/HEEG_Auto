...
9. 窗口和元素资产应优先补稳定 `AutomationId`；执行层会优先使用严格定位，失败后基于 `AutomationId` 做放宽匹配，并对无标题弹窗按钮走全局顶层窗口兜底查找。

## 最近更新

### 2026-05-09

- 补充 UIA 弹窗和控件定位容错口径：`AutomationId` 是首选稳定标识，但执行层仍需处理 WPF 弹窗层级、控件 `Name` 变化和 UIA 暴露延迟。
- 补充历史回放剪辑 Jenkins 失败修复结论：剪辑完成弹窗的 `OKButton` 改为可在默认 `20` 秒 watchdog 下稳定确认。
- 补录本次代码改动必须同步文档的治理要求执行结果。
